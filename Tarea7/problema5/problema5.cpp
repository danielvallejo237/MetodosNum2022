...
double computeAlpha(Matrix A, vector<double> residual, vector<double> p)
{
  double denom=residual*residual;
  vector<double> aux;
  aux=A*p;
  double num=p*aux;
  return denom/num;
}
void ConjugateGradient(Matrix A, Matrix b, vector<double> &x,double tol=1e-8)
{
  //Convertimos primero el vector b a un vector que sera nuestra solucion inicial
  vector<double> bvec=b.ToVector();
  vector<double> residual,p;
  residual=bvec-A*x;
  p=residual;
  double alpha,beta;
  ...
  while (error>tol)
  {
    alpha=computeAlpha(A,residual,p);
    x=x+p*alpha;
    ...
    {
      beta=(rtmp*rtmp)/(residual*residual);
      p=rtmp+p*beta;
      residual=rtmp;
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  d.print_to_text(name+"_Soluciones_GradienteConjugado.txt");
