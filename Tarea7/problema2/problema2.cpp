...
pair<Matrix,Matrix> SubspaceIteration(Matrix A,int nvals,double tol=1e-20, int Mit=10000)
{
  pair<Matrix,pair<Matrix,Matrix>> F=LDUFactorization(A);
  Matrix G,H,K;
  G=F.first;
  H=F.second.first;
  K=F.second.second;
  ChangeOffDiagonalSign(G,1);
  ChangeOffDiagonalSign(K,2);
  InverseDiagonal(H);
  //G.print_content();
  Matrix MM=MultiplyMat(K,MultiplyMat(H,G));
...
int main(int argc, char * argv[])
{
  ...
  string n1=name+"_Eigenvals_SubspaceInvIt.txt";
  string n2=name+"_Eigenvecs_SubspaceInvIt.txt";
