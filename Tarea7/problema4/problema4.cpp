/* Implementación del método QR para solución de un sistema de ecuaciones */
#include<bits/stdc++.h>
#include<cmath>
#include<fstream>
#include<sstream>
...
int main(int argc, char *argv[])
{
  ...
  Matrix Sol=SolveQR(A,b);
  auto end = chrono::steady_clock::now();
  double elapsed_seconds = std::chrono::duration_cast<std::chrono::duration<double> >(end - start).count();
  cout << "Tiempo de ejecución en segundos: "<<elapsed_seconds<<" segs"<<endl;
  string name=string(argv[1]);
  name.erase(name.end()-4,name.end());
  Sol.print_to_text(name+"_Soluciones_QR.txt");
  cout<<"Error ||Ax-b||: "<<Norm((A*Sol.ToVector()-b.ToVector()))<<endl;
  return 0;
}
