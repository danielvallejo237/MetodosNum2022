/* Implementación del método de gradiente conjugado para solución del sistema de ecuaciones */
#include<bits/stdc++.h>
#include<cmath>
#include<fstream>
#include<sstream>
...
double computeAlphaPreconditioned(Matrix A, vector<double> residual, vector<double> p,vector<double> z)
{
  double denom=residual*z;
  vector<double> aux;
  aux=A*p;
  double num=p*aux;
  return denom/num;
...
Matrix JacobiPrecond(Matrix A)
{
  Matrix ICF(A.n,A.m);
  for(int i=0;i<A.n;i++)
  {
    ICF.put(i,i,1/A.get(i,i));
  }
  return ICF;
}

void PreconditionedConjugateGradient(Matrix A, Matrix b, vector<double> &x,double tol=1e-7)
{
  //Convertimos primero el vector b a un vector que sera nuestra solucion inicial
  vector<double> bvec=b.ToVector();
  Matrix Precondicion;
  Precondicion=JacobiPrecond(A);
  vector<double> residual,p,z,ztmp;
  residual=bvec-A*x;
  z=Precondicion*residual;
  p=z;
  double alpha,beta;
  ...
  while (error>tol)
  {
    alpha=computeAlphaPreconditioned(A,residual,p,z);
    x=x+p*alpha;
    rtmp=residual-(A*p)*alpha;
    error=Norm(rtmp);
    if (error<tol) break;
    else
    {
      ztmp=Precondicion*rtmp;
      beta=(rtmp*ztmp)/(residual*z);
      p=ztmp+p*beta;
      residual=rtmp;
      z=ztmp;
    }
...
int main(int argc, char *argv[])
{
  ...
  Matrix b(argv[2]);
  Matrix c(argv[3]);
  vector<double> x=c.ToVector();
  auto start = chrono::steady_clock::now();
  PreconditionedConjugateGradient(A,b,x);
  ...
  d.print_to_text(name+"_Soluciones_GradienteConjugadoPrecondicionado.txt");
  cout<<"Error ||Ax-b||: "<<Norm((A*d.ToVector()-b.ToVector()))<<endl;
  return 0;
}
