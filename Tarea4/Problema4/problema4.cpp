/*
Solución del método de Jacobi para resolver un sistema de ecuaciones de forma iterativa
...
pair<Matrix,bool> JacobiMethod(Matrix A, Matrix b, Matrix &XO, int MaxIter=200, double TOL=1e-8)
{
...
int main(int argc, char *argv[])
{
  ...
  pair<Matrix,bool> Sol=JacobiMethod(A,b,XO);
  end = std::chrono::system_clock::now();
  std::chrono::duration<double> elapsed_seconds = end - start;
  std::time_t end_time = std::chrono::system_clock::to_time_t(end);
  std::cout << "Tiempo transcurrido: " << elapsed_seconds.count() << "s\n";
  if (Sol.second) Sol.first.print_to_text("SalidaJacobi.txt");
  if (argc>3)
  {
    Matrix Comp(argv[3]);
